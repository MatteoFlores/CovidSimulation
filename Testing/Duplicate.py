...
class CVirus:
    def __init__(self):
        ...
        self.infectedGrid = np.zeros([self.nx, self.ny], dtype=bool)
        self.numInfected = 0
        ...
        self.ax01 = pylab.subplot2grid((2, 2), (0, 0))
        self.ax02 = plt.subplot()
        self.ax03 = pylab.subplot2grid((2, 2), (1, 0), colspan=2, rowspan=1)
        self.ax04 = pylab.subplot2grid((2, 2), (0, 0))
        #self.ax04 = plt.subplots()
        self.ax01.set_title('Population Density')
        self.ax02.set_title('Infection Spread')
        self.ax03.set_title('Total Currently Infected')
        # set y-limits
        ...
        self.p022 = self.ax03.imshow([[self.grid[x][y].infected for x in range(self.nx)] for y in range(self.ny)], cmap = 'Blues', vmin=0, vmax=1)
        self.Originalim = plt.imshow([[self.grid[x][y].infected for x in range(self.nx)] for y in range(self.ny)], cmap='gist_gray_r', vmin=0, vmax=1)
    ...
    def init(self):
        ...
        self.p022.set_data(np.zeros((self.nx, self.ny)))
    ...
    def anim(self,day):
        ...
        for i,j in np.ndindex(self.grid.shape):
            if(self.grid[j,i].infected == True and self.grid[i][j].daysInfected < 26):
                self.grid[i,j].daysInfected +=1
                self.grid[i][j].infectedPopulation += int((self.grid[i][j].population / self.maxPopulation) * spreadRate*self.grid[i][j].daysInfected)#spreadrate to the power of the days infected
                ...
                if(self.grid[i][j].daysInfected > 25):
